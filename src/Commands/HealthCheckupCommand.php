...
class HealthCheckupCommand extends Command
{
    ...
    public function handle(): int
    {
        if (PHP_OS_FAMILY == "Linux") {
            $this->components->task("<fg=black;bg=bright-yellow;options=bold> {$this->module} </> Verify storage directory permission", function () {
                shell_exec('sudo chown -R www-data:www-data /var/www/html/storage');
                shell_exec('sudo chmod -R 777 /var/www/html/storage');
            });
        }

        $this->components->task("<fg=black;bg=bright-yellow;options=bold> {$this->module} </> Clear cached bootstrap files", function () {
            Artisan::call('optimize:clear --quiet');
        });

        $this->components->task("<fg=black;bg=bright-yellow;options=bold> {$this->module} </> Publish default assets", function () {
            Artisan::call('vendor:publish --tag=fintech-auth-assets --quiet --force');
        });

        $this->components->task("<fg=black;bg=bright-yellow;options=bold> {$this->module} </> Publish file manager assets", function () {
            Artisan::call('vendor:publish --tag=fm-assets --quiet --force');
        });

        $this->components->task("<fg=black;bg=bright-yellow;options=bold> {$this->module} </> Broadcast queue restart signal", function () {
            Artisan::call('queue:restart --quiet');
        });

        $this->components->task("<fg=black;bg=bright-yellow;options=bold> {$this->module} </> Verify scheduler log file", function () {
            if (!file_exists(storage_path('/logs/scheduler.log'))) {
                @file_put_contents(storage_path('/logs/scheduler.log'), '');
            }
        });

        $this->components->task("<fg=black;bg=bright-yellow;options=bold> {$this->module} </> Verify queue worker log file", function () {
    ...
    private function checkAvailablePackages(): void
    {
        foreach (config('fintech.core.packages', []) as $code => $package) {
            $this->components->twoColumnDetail(
                "<fg=black;bg=bright-yellow;options=bold> {$package} </> API routes",
